...
const EFI_GRAOHICS_OUTPUT_PROTOCOL_GUID: EfiGuid = EfiGuid {
    data0: 0x9042a9de,
    data1: 0x23dc,
    data2: 0x4a38,
...
fn locate_graphics_protocol<'a>(
    ...
    let status = (efi_system_table.boot_services.locate_protocol)(
        &EFI_GRAOHICS_OUTPUT_PROTOCOL_GUID,
        null_mut::<EfiVoid>(),
...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    let cy = rect_size / 2;
    for i in (0..=rect_size).step_by(grid_size as usize) {
        let _ = draw_line(&mut vram, 0x00ff00, cx, cy, 0, i);
...
fn calc_slope_point(da: i64, db: i64, ia: i64) -> Optipn<i64> {
    if da < db {
        None
    } else if da == 0 {
        Some(0)
...
fn draw_line<T: Bitmap>(buf: &mut T, color: u32, x0: i64, y0: i64, x1: i64, y1: i64) -> Result<()> {
    ...
    let mut sx = (x1 - x0).signum();
    let mut dy = (y1 - y0).abs();
    let mut sy = (y1 - y0).signum();
    if dx >= dy {
